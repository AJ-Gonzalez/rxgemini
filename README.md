...
| LOG_MODE       | Switch between output modes: `pretty`: uses colorful and detailed messages, `pretty_timestamped`: uses colorful and detailed messages with a timestamp, `python`: uses the python logger, `quiet` shows no output unless necessary to prompt ot inform the user | String | `LOG_MODE: "pretty"`|
